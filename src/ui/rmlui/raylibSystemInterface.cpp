...
static double keyRepeatTimer = 0.0;
...
{
    const char *output = TextFormat("[RmlUi] %s", message.c_str());
    switch (type)
    {
    case Rml::Log::LT_ERROR:
        TraceLog(LOG_ERROR, "%s", output);
        break;
    case Rml::Log::LT_ASSERT:
        TraceLog(LOG_TRACE, "%s", output);
        break;
    case Rml::Log::LT_WARNING:
        TraceLog(LOG_WARNING, "%s", output);
        break;
    case Rml::Log::LT_INFO:
        TraceLog(LOG_INFO, "%s", output);
        break;
    case Rml::Log::LT_DEBUG:
        TraceLog(LOG_DEBUG, "%s", output);
        break;
    default:
        break;
    }
...
void RaylibSystemInterface::HandleKeyboardEvents(Rml::Context *context)
{
    ...
    {
        KeyboardKey key = entry.first;
        if (IsKeyPressed(key))
        {
            context->ProcessKeyDown(entry.second, ConvertKeyModifiers());
            keyRepeatTimer = 0.5;
            break;
        }

        if (IsKeyDown(key) && keyRepeatTimer <= 0.0)
            context->ProcessKeyDown(entry.second, ConvertKeyModifiers());

        if (IsKeyReleased(key))
        {
            context->ProcessKeyUp(entry.second, ConvertKeyModifiers());
            keyRepeatTimer = 0.5;
        }
    }
